    ...
    public function init()
    {
        ...
        if (!$this->option['pay_all']) {
            $this->add(array(
                ...
                'attributes' => array(
                    'value'=> '<div class="mt-2 mb-3">' .  sprintf(__('You chose to pay through 2 installments : %s € now, and %s € before %s'), number_format($this->option['due_price'] * $this->option['composition'][0] / 100, 2, '.', ''), number_format($this->option['due_price'] - number_format($this->option['due_price'] * $this->option['composition'][0] / 100, 2, '.', ''), 2, '.', ''), _date($this->option['limit_date'])) . '</div>',
                ),
            ));
        }
        ...
        if ($this->config['order_payment'] == 'payment') {
            $title = sprintf(__('Pay %s €'), number_format($this->option['due_price'] * $this->option['composition'][0] / 100, 2, '.', ''));
        } else {
            $title = __('Save order');
        }
