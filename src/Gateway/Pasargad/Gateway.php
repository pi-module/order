...
class Gateway extends AbstractGateway
{
    ...
    public function setRedirectUrl()
    {
        ...
        $data1     = "#" . $merchantCode . "#" . $terminalCode . "#" . $invoiceNumber . "#" . $invoiceDate . "#" . $amount . "#" . $redirectAddress . "#"
            . $action . "#" . $timeStamp . "#";
    ...
    public function verifyPayment($request, $processing)
    {
        ...
        if ($checkResult["resultObj"]['result'] && $checkResult["resultObj"]['transactionReferenceID'] == $request['tref']) {
            ...
            // Set signature
            $processor      = new \RSAProcessor($certificate, \RSAKeyType::XMLFile);
            $data           = "#" . $fields['MerchantCode'] . "#" . $fields['TerminalCode'] . "#" . $fields['InvoiceNumber'] . "#" . $fields['InvoiceDate']
                . "#" . $fields['amount'] . "#" . $fields['TimeStamp'] . "#";
            $data           = sha1($data, true);
            $data           = $processor->sign($data);
            $fields['sign'] = base64_encode($data);
            // Set verify result
            $sendingData  = "MerchantCode=" . $this->gatewayOption['merchantCode'] . "&TerminalCode=" . $this->gatewayOption['terminalCode'] . "&InvoiceNumber="
                . $invoice['random_id'] . "&InvoiceDate=" . date("Y/m/d H:i:s", $invoice['time_create']) . "&amount=" . intval($invoice['total_price'])
                . "&TimeStamp=" . $timeStamp . "&sign=" . $fields['sign'];
